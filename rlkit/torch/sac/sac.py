...
# agent.networks: [self.context_encoder, self.policy]
    ...
    def pretrain(self, indices):
        ...
        for i in range(num_updates):
            ...
            mini_batch_ = [x[:, i * mb_size: i * mb_size + mb_size, :] for x in  batch_]
    ...
    def _pre_take_step(self, indices, context1, context1_):
        ...
        loss = self.ce_coeff * self.cross_entropy_loss(logits, labels)
    ...
    def _take_step(self, indices, context1, context1_, exp=False):
        ...
        ce_loss = self.ce_coeff * cross_entropy_loss
        ...
        if self.eval_statistics is None:
            ...
            self.eval_statistics['Contrastive Loss'] = np.mean(ptu.get_numpy(cross_entropy_loss))
        ...
        return {"cross_entropy_loss": cross_entropy_loss.item(),
    ...
    def _take_step_exp(self, indices, context):
        ...
        return {"exp_qf_loss": qf_loss.item(),
                "exp_vf_loss": vf_loss.item(),
                "exp_policy_loss": policy_loss.item(),
                "exp_log_pi": log_pi.mean().item()}
